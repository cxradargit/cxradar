    ...
    <div className="p-8 max-w-5xl mx-auto cx-fade-up">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
        ...
        <button
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, color: 'white' }}
        >
        ...
        <div className="bg-white border rounded-xl p-6 mb-6" style={{ borderColor: '#E2E8F0' }}>
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', color: '#64748B', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.04em', marginBottom: '6px' }}>Nome</label>
                <input
              ...
              <div>
                <label style={{ display: 'block', color: '#64748B', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.04em', marginBottom: '6px' }}>Tipo principal</label>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                className="cx-btn-gradient"
                style={{ padding: '8px 20px', borderRadius: '8px', border: 'none', fontSize: '0.875rem', fontWeight: 600, color: 'white', cursor: creating || !nome.trim() ? 'not-allowed' : 'pointer', opacity: creating || !nome.trim() ? 0.6 : 1 }}
              >
      ...
      {surveys.length === 0 && !showForm && (
        <div style={{ background: 'white', border: '1px dashed #E2E8F0', borderRadius: '12px', padding: '64px', textAlign: 'center' }}>
          <ClipboardList style={{ width: '40px', height: '40px', color: '#E2E8F0', margin: '0 auto 12px' }} />
          <p style={{ color: '#94A3B8', fontSize: '0.875rem', fontWeight: 500 }}>Nenhuma pesquisa ainda.</p>
          <button
            onClick={() => setShowForm(true)}
            className="cx-btn-gradient"
            style={{ marginTop: '16px', display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, color: 'white' }}
          >
            <Plus style={{ width: '16px', height: '16px' }} />
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #F1F5F9', background: '#F8FAFC' }}>
                {['Nome', 'Tipo', 'Score Médio', 'Status', 'Criada em', ''].map(h => (
                  <th key={h} className="text-left" style={{ padding: '12px 24px', fontSize: '0.65rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#64748B', fontWeight: 600 }}>
                    {h}
                  </th>
                ))}
              </tr>
                  ...
                  >
                    <td style={{ padding: '16px 24px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '10px', color: '#94A3B8', background: '#F1F5F9', padding: '2px 5px', borderRadius: '4px', flexShrink: 0 }}>
                          #{shortCode}
                        </span>
                    ...
                    <td style={{ padding: '16px 24px', color: '#94A3B8', fontSize: '0.8125rem' }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        cursor: disabled ? 'not-allowed' : 'pointer', color: danger ? '#EF4444' : '#94A3B8',
      ...
      onMouseLeave={e => {
        ...
        ;(e.currentTarget as HTMLElement).style.color = danger ? '#EF4444' : '#94A3B8'
      }}
    >
      {children}
    </button>
