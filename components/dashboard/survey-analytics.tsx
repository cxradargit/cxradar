...
import { ArrowLeft, Users, MessageSquare, Percent, TrendingUp, Clock, ChevronRight } from 'lucide-react'
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 cx-fade-up">
      ...
      {loading && !data && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
          {[...Array(4)].map((_, i) => (
            <div key={i} className="animate-pulse" style={{ height: '96px', background: 'white', borderRadius: '12px', border: '1px solid #E2E8F0' }} />
          ))}
        </div>
      )}
      ...
      {data && (
        <div className="space-y-6">
            ...
            <div className="bg-white border rounded-xl p-6" style={{ borderColor: '#E2E8F0' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0' }}>
                <FunnelStep
                  ...
                  bg="#EFF6FF"
                />
                <ChevronRight style={{ color: '#E2E8F0', width: '20px', height: '20px', flexShrink: 0 }} />
                <FunnelStep
                  label="Respondidos"
          ...
          {data.npsData && data.npsData.total > 0 && (
            <div className="bg-white border rounded-xl" style={{ borderColor: '#E2E8F0', borderLeft: '3px solid #2563EB', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '24px' }}>
              <div>
...
function FunnelStep({ label, value, pct, color, bg }: { label: string; value: number; pct: number; color: string; bg: string }) {
  return (
    <div style={{ flex: 1, background: bg, border: `1px solid ${color}20`, borderRadius: '10px', padding: '16px 20px' }}>
...
function KPICard({ label, value, icon: Icon }: {
  ...
  return (
    <div className="bg-white border rounded-xl" style={{ borderColor: '#E2E8F0', borderLeft: '3px solid #2563EB', padding: '20px' }}>
    ...
    content = (
      <div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {visible.map((t, i) => (
            <div key={i} style={{ fontSize: '0.875rem', color: '#64748B', background: '#F8FAFC', borderRadius: '8px', padding: '10px 14px', lineHeight: 1.6, border: '1px solid #F1F5F9' }}>
              "{t}"
            </div>
          ))}
        </div>
